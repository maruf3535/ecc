...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $notice_question = $_POST['notice-question'];
    ...
    if ($is_student) {
        ...
        $sql = "INSERT INTO `notice_reply` (`notice_id`, `student_id`, `teacher_id`, `notice_reply`, `time`) VALUES ('$notice_id', '$std_id_main', '0', '$notice_question', current_timestamp())";
    }
    if ($is_teacher) {
        $tec_id_main = $_SESSION['tec_id'];
        $tec_first_name = $_SESSION['tec_first_name'];
        $sql = "INSERT INTO `notice_reply` (`notice_id`, `student_id`, `teacher_id`, `notice_reply`, `time`) VALUES ('$notice_id', '0', '$tec_id_main', '$notice_question', current_timestamp())";
    }
...
<body>
    ...
    <div class="container my-3">
        ...
        $notice_id = $_GET['notice_id'];
        $sql = "SELECT * FROM `noticelist` WHERE `notice_id` = '$notice_id'";
        $result = mysqli_query($connection, $sql);
        ...
        <?php
        $result = $connection->query("SELECT photo FROM teachers WHERE `teacher_id` = '$tec_id'");
        ?>
        ...
        <?php
        echo '<h5 class="m-0 text-center">' . $row_data['teacher_name'] . ' Sir</h5>
                    <p class="m-0" style="font-size: 14px;">' . date("F j, Y, g:i a", strtotime($row_data['time'])) . '</p>
                </div>
                <div class="col-lg-10 col-sm-12">
                    <h4 class="alert-heading ">' . $row_data['notice_title'] . '</h4>
                    <p>' . $row_data['notice_description'] . '</p>
                </div>
            </div>
        </div>';
        ?>
                        ...
                        <div id="flush-collapseOne" class="accordion-collapse collapse" aria-labelledby="flush-headingOne" data-bs-parent="#accordionFlushExample">
                            <div class="accordion-body">
                                <form action="<?php $_SERVER['REQUEST_URI'] ?>" method="POST">
                                    <div class="mb-3">
                                        <label for="notice-question" class="form-label">Question *</label>
                                        <textarea class="form-control" id="notice-question" name="notice-question" rows="3" required></textarea>
                                    </div>
        ...
        <div class="row">
            ...
            <div class="col-lg-6 col-md-8 col-10">
                ...
                $sql = "SELECT * FROM `notice_reply` WHERE `notice_id` = '$notice_id'";
                $result = mysqli_query($connection, $sql);
